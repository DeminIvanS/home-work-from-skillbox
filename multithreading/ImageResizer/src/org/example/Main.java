...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        if (files.length <= 3) {
            org.example.ImageResizer resizer = new org.example.ImageResizer(files, newWidth, dstFolder);
        ...
        if (files.length > 3 && files.length <= countThreads / 2) {
            ...
            org.example.ImageResizer resizer = new org.example.ImageResizer(files1[0], newWidth, dstFolder);
            new Thread(resizer).start();
            files1[1] = new File[files.length - files1[0].length];
            System.arraycopy(files, 0, files1[1], 0, (files.length - files1[0].length));
            org.example.ImageResizer resizer1 = new org.example.ImageResizer(files1[1], newWidth, dstFolder);
            ...
            for (int i = 1; i <= countThreads; i++) {
                ...
                if (files.length - ((countThreads - (i - 1)) * imagePerThreading) > 0 && totalLength > 0) {
                    ...
                    org.example.ImageResizer resizer = new org.example.ImageResizer(files1[j], newWidth, dstFolder);
                    new Thread(resizer).start();
                    break;
                    ...
                    for (; j <= createCountThreads; j++) {
                        ...
                        org.example.ImageResizer resizer = new org.example.ImageResizer(files1[j - 1], newWidth, dstFolder);
                        new Thread(resizer).start();
